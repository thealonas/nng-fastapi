...
class AdminService:
    ...
    async def promote_to_admin(self, admin_id: int, user_id: int) -> bool:
        try:
    ...
    async def demote_from_admin(self, admin_id: int, user_id: int) -> bool:
        try:
            user = self.postgres.users.get_user(user_id)
            user.admin = False
            self.postgres.users.update_user(user)
    ...
    async def bulk_update_users(
        self, admin_id: int, user_ids: List[int], updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        successful = 0
        failed = 0
        ...
        return {"total": len(user_ids), "successful": successful, "failed": failed}
    ...
    async def search_admin_logs(self, query: str, limit: int = 50) -> List[AdminAction]:
        ...
        results = [
            ...
            if (
                query_lower in a.action_type.lower()
                or query_lower in a.target_type.lower()
                or (a.target_id and query_lower in a.target_id.lower())
            )
        ]
