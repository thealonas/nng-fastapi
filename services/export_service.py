...
class ExportService:
    ...
    def _extract_fields(
        self, item: Any, fields: Optional[List[str]], date_format: str
    ) -> Dict[str, Any]:
    ...
    def _to_json(self, items: List[Any], config: ExportConfig) -> str:
        data = [
            self._extract_fields(item, config.fields, config.date_format)
            for item in items
        ]
    ...
    def _to_csv(self, items: List[Any], config: ExportConfig) -> str:
        if not items:
            return ""
    ...
    def _to_xml(self, items: List[Any], config: ExportConfig) -> str:
        data = [
            self._extract_fields(item, config.fields, config.date_format)
            for item in items
        ]
    ...
    async def export_data(
        self, items: List[Any], prefix: str, config: Optional[ExportConfig] = None
    ) -> ExportResult:
        if config is None:
            config = ExportConfig()
    ...
    async def export_groups(
        self, config: Optional[ExportConfig] = None
    ) -> ExportResult:
    ...
    async def export_watchdog_logs(
        self, config: Optional[ExportConfig] = None
    ) -> ExportResult:
    ...
    async def export_user_data(
        self, user_id: int, config: Optional[ExportConfig] = None
    ) -> ExportResult:
