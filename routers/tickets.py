...
async def algolia_search(
    ...
    if cached:
        await metrics_service.increment("cache_hits", labels={"endpoint": "algolia_search"})
        return cached
    ...
    await logging_service.info(f"Algolia search: '{query.query}' returned {len(results)} results")
...
async def update_status(
    ...
    try:
        ...
        await metrics_service.increment("ticket_status_updates", labels={"status": str(status.status)})
...
async def add_ticket(
    ...
    try:
        ...
        await metrics_service.increment("tickets_created", labels={"type": str(issue.type)})
        await logging_service.info(f"Ticket created: {ticket.ticket_id} by user {issue.user_id}")
        cache_instance.delete("opened_tickets", namespace="tickets")
