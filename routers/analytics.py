...
async def get_user_analytics(
    ...
    active_rate = (
        (snapshot.active_users / snapshot.total_users * 100)
        if snapshot.total_users > 0
        else 0
    )
    banned_rate = (
        (snapshot.banned_users / snapshot.total_users * 100)
        if snapshot.total_users > 0
        else 0
    )
...
async def record_metric(
    ...
    metric_type = (
        ...
        if metric.metric_type in [e.value for e in MetricType]
...
async def get_dashboard_data(
    ...
    return {
        "summary": {
            ...
            "groups": {"total": snapshot.total_groups},
            "support": {
                "open_tickets": snapshot.open_tickets,
                "pending_requests": snapshot.pending_requests,
            },
